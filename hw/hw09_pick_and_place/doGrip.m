...
%--------------------------------------------------------------------------
    ...
    % Potentially incorporate similarly to below
    %
    % cur_q = get_current_joint_states(optns);
    % cur_gripper_q = cur_q(2);
    % if cur_gripper_q == grip_goal.Trajectory.Position
    %      %%end file without sending
    % else
    %   continue with below code
